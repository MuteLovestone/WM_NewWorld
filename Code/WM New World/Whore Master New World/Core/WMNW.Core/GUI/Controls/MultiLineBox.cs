...
namespace WMNW.Core.GUI.Controls
{
    public class MultiLineBox:ControlBase
    {
        ...
        public override void Update( GameTime gameTime )
        {
            ...
            var i = ( int )( ( Size.Y ) / GraphicsHandler.MesureString ( Font, Text.Split ( '\n' ) [ 0 ] ).Y );
            ...
            for ( var mls = 0; mls < _multiLineString.Length; mls++ )
            {
                ...
                string changingText =
                    _multiLineString.Where ( multiLine => multiLine != "" ).Aggregate ( Text, (current, multiLine ) => current.Replace ( multiLine, "" ) );
                ...
                if ( changingText.Contains ( @"\n" ) )
                {
                    ...
                    if ( index > 0 )
                        changingText = changingText.Substring ( 0, index );
